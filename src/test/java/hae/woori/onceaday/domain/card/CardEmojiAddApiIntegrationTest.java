...
class CardEmojiAddApiIntegrationTest {
    ...
    void addEmoji_whenFirstTime_success() throws Exception {
        ...
        EmojiDocument emoji = EmojiDocument.builder()
                .emojiUrl("url1")
                .build();
        emoji = emojiDocumentRepository.save(emoji);

        EmojiAddDto.Request request = new EmojiAddDto.Request("user123", card.getId(), emoji.getId());
    ...
    void addEmoji_whenAlreadyExists_replacesEmoji() throws Exception {
        UserDocument user = UserDocument.builder()
                ...
                .build();
        userDocumentRepository.save(user);
		EmojiDocument oldEmoji = EmojiDocument.builder()
			.emojiUrl("oldUrl")
			.build();
        ...
        EmojiAddDto.Request request = new EmojiAddDto.Request("user123", card.getId(), newEmoji.getId());
        ResultActions result = mockMvc.perform(post("/api/v1/card/emoji/add")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)));
    ...
    void addEmoji_whenCardNotExists_fail() throws Exception {
        ...
        EmojiAddDto.Request request = new EmojiAddDto.Request("user123", "notExistCardId", "emoji1");
        ResultActions result = mockMvc.perform(post("/api/v1/card/emoji/add")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)));
    ...
    void addEmoji_whenEmojiNotExists_fail() throws Exception {
        ...
        EmojiAddDto.Request request = new EmojiAddDto.Request("user123", card.getId(), "notExistEmojiId");
        ResultActions result = mockMvc.perform(post("/api/v1/card/emoji/add")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)));
    ...
    void addEmoji_whenUserNotExists_fail() throws Exception {
        ...
        EmojiAddDto.Request request = new EmojiAddDto.Request("notExistUserId", card.getId(), "emoji1");
        ResultActions result = mockMvc.perform(post("/api/v1/card/emoji/add")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)));
